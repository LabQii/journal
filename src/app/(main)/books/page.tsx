...
// ─── Modal animation variants (outside render — stable references) ──
...
// ─── Book Form state (useReducer = 1 reconcile per update) ───────────
interface FormState {
    ...
    return (
        <div className="group relative animate-fadeIn">
            ...
            >
                ...
                {/* Always render the favorite button if logged in (canCreate) */}
                {canCreate && (
                    <button
...
export default function BooksPage() {
    ...
    // Collapsed form state — 1 reconcile per field change instead of 7
    ...
    const handleToggleFavorite = useCallback(async (book: Book, e: React.MouseEvent) => {
        ...
        if (!canCreate) return; // Must be logged in to favorite
        ...
        try {
            ...
            console.error(error);
            // Revert on error
            // (In a real app, fetching fresh data is safer, but we can do a simple toggle back)
            startTransition(() => {
                setBooks(prev => {
    ...
    const handleCreateBook = useCallback(async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                // Non-urgent list update — browser paints modal-close first
                startTransition(() => {
                    setBooks(prev => {
    ...
    return (
        <div className="w-full max-w-[1600px] mx-auto px-4 md:px-6 lg:px-10 mt-8 space-y-12 min-h-screen pb-20">
            ...
            {/* Hero — CSS animation instead of motion.div; no Framer on the critical path */}
