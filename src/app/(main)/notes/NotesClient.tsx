...
import { useState, useCallback, useReducer, startTransition, memo, useDeferredValue, useMemo } from "react";
...
const NoteCard = memo(function NoteCard({
    note, canUpdate, canDelete, onEdit, onDelete, priority,
}: {
    ...
    return (
        <div className="group relative bg-card/40 backdrop-blur-sm border border-border hover:border-primary/40 rounded-2xl overflow-hidden hover:shadow-lg transition-all duration-300 flex flex-col animate-fadeIn">
            ...
            {/* Thumbnail — aspect-video reserves space, no layout shift */}
                    ...
                    <>
                        ...
                        >
                            ...
                            {canUpdate && (
                                ...
                                >
                                    <div className="p-2 rounded-xl bg-primary/10"><Pencil className="h-4 w-4 text-primary" /></div>
                            ...
                            {canDelete && (
                                ...
                                >
                                    <div className="p-2 rounded-xl bg-rose-100 dark:bg-rose-900/30"><Trash2 className="h-4 w-4 text-rose-500" /></div>
...
export default function NotesClient({ initialNotes }: { initialNotes: Note[] }) {
    ...
    // Defer search so input paints instantly on every keystroke
    ...
    // Memoized — only recomputes when deferred values change
