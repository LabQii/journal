...
// ─── Add-form state (useReducer = 1 reconcile instead of 6) ─────────
...
export default function GalleryPage() {
    ...
    // Form state via reducers (1 reconcile pass per update instead of 6)
    ...
    const fetchImages = useCallback(async () => {
        try {
            ...
            if (res.ok) {
                ...
                startTransition(() => setImages(data));
            }
    ...
    const handleToggleFavorite = useCallback(async (img: GalleryImage) => {
        if (!canCreate) return; // Must be logged in
    ...
    const handleAddImage = useCallback(async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                startTransition(() => {
                    setImages(prev => {
                        return [...prev.filter(b => b.isFavorite), newImage, ...prev.filter(b => !b.isFavorite)];
                    });
                });
                setIsAdding(false);
